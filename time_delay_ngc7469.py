# -*- coding: utf-8 -*-
"""
Created on Sun Aug 23 16:21:15 2015
...
MIN_LOG_NU = 12.5
MAX_LOG_NU = 16
MIN_LOG_R = 11.3 #Moves SED to right/left (higher frequency)
MAX_LOG_R = 20.5
MIN_LOG_R_EMM = 11.3 #Moves SED to right/left (higher frequency)
MAX_LOG_R_EMM = 20.5


log_lum_norm = (43.56-14.77) #point on observed SED used to normalize
log_nu_norm = 14.77
...
"""
#Emmissivity Graphs
log_r_emm = numpy.arange(MIN_LOG_R_EMM,MAX_LOG_R_EMM, dlog_r)
log_r_femm = map(lambda x: x+log_r_scale, log_r_emm)
"""
...
wavelength = [1315, 1485, 1740, 1825, 4945, 5100, 6962]
colors = ['r', 'g', 'c', 'm', 'y', 'b', 'p']
        ...
        if v <= 0.0:
            for x in xrange(len(diff)):
                if v == diff[x]:
                    ...
                    max_log_r_arr_new.append(max_log_r_LD)
                    print "Log R(" + str(wavelength[num]) + ")" "= " + str("{:6.2f}".format(log_r_max+log_r_scale)) + " =" + str("{:6.2f}".format(max_log_r_LD)) + " ld" + str("{:6.2f}".format((max_log_r_LD)-(max_log_r_arr_new[0])))
            break        
        else:
            continue
    #Plotting E(log R)
...
#data from kriss et. al. 2000
lambda_7469 = [1315,1485, 1740, 1825, 4945, 6962]
tau_7469 = [0.00, 0.09, 0.28, 0.36, 1.17, 1.68]
tau_err_pos_7469 = [0.09, 0.11, 0.12, 0.11, 0.55, 1.12]
tau_err_neg_7469 = [0.09, 0.08, 0.13,0.17, 0.33, 0.82]
tau_err_7469 = [tau_err_pos_7469, tau_err_neg_7469]
plt.errorbar(lambda_7469, tau_7469, yerr=tau_err_7469, fmt = 'o', color = 'k')
#plt.plot(wavelength, max_log_r_arr, 'k')
plt.ylabel("Lag (days)")
plt.xlabel("Wavelength (\r{A})")

x_smooth = numpy.linspace(wavelength[0],wavelength[-1], 200)
y_smooth = spline(wavelength, max_log_r_arr_new, x_smooth)
#plt.plot(x_smooth, y_smooth, 'k')
#plt.plot(wavelength, max_log_r_arr_new, 'k.')
plt.xlim(1000, 7500)
        ...
        if v <= 0.0:
            for x in xrange(len(diff)):
                if v == diff[x]:
                    ...
                    ar_pl_max = emm_arr[x]
                    max_log_r_LD = (10**(log_r_max+log_r_scale))/(2.59*10**15)
                    max_log_r_arr_classic.append(max_log_r_LD)
                    print "Log R(" + str(wavelength[num]) + ")" "= " + str("{:6.2f}".format(log_r_max+log_r_scale)) + " =" + str("{:6.2f}".format(max_log_r_LD)) + " ld" + str("{:6.2f}".format((max_log_r_LD)-(max_log_r_arr_classic[0])))
            break        
        else:
            continue
    #Plotting E(log R)
...
#x_smooth = numpy.linspace(wavelength[0],wavelength[-1], 200)
#y_smooth = spline(wavelength, max_log_r_arr_classic, x_smooth)
#plt.plot(x_smooth, y_smooth, 'k--')
#plt.plot(wavelength, max_log_r_arr_classic, 'k.')
plt.xlim(0,8000)
#plt.show()
...
plt.savefig('timedelay_7469_fig.pdf', bbox_inches='tight')
plt.show()

log_nu_slope = []
log_lum_slope = []

new_classic_factor = map(lambda x,y: x/y, max_log_r_arr_new, max_log_r_arr_classic)

plt.plot(wavelength,new_classic_factor, 'k')
plt.ylabel("Factor")
plt.xlabel("Wavelength (\r{A})")
plt.savefig('factor_7469.eps', bbox_inches='tight')
plt.show()